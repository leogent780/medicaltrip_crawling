...
### 서울 전체 피부과 리스트업 (`--regions 전체`)
에스테틱처럼 상권 몇 곳만 도는 게 아니라 "서울 피부과 전체"처럼 도시 전역에 흩어진
업종을 훑고 싶으면 `--regions`에 `전체`(또는 `서울전체`/`all`) 하나만 넘긴다. 서울
25개 자치구(강남구~중랑구) 전부를 자치구별 동네 별칭까지 섞어서 순회한다.
```bash
python naver_place_scraper.py --regions 전체 --keywords 피부과 --max-per-region 20 \
  --output seoul_dermatology_list.xlsx
```
자치구가 넓은 만큼 25곳 x 자치구당 3~8개 별칭이라 호출 수가 많아지고, 시간도 꽤
걸린다(요청 사이 지연을 임의로 줄이지 말 것 — 아래 주의사항 참고). 웹 화면에서는
"추가 지역" 입력칸에 `전체`라고만 입력해도 동일하게 동작한다.
...
- 이 코드는 실제 네이버 서버에 접속해 테스트하지 못한 상태로 작성됐다(작업 환경 네트워크
  정책상 `search.naver.com` 등 외부 접속이 막혀 있었음 — 서울 전체 자치구/피부과
  버전도 동일). 로직은 가짜 응답과 지역 별칭 조합만 단위 테스트했고, 실제 응답 형식과
  다를 수 있다. 네이버 접속이 되는 로컬 환경에서 돌려보고 안 되는 부분이 있으면 알려주면 된다.
